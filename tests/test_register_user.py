...
from test_data.register_user_data import (register_endpoint, register_payload, register_payload_without_password,
                                          register_success_response_body, register_unsuccess_response_body)
...
def test_register_successful():
    ...
    with step("Проверка значения response body"):
        assert response_body == register_success_response_body
    with step("Проверка схемы ответа"):
        validate(response_body, schema=register_user_schemas.post_register_successful)
...
def test_register_unsuccessful():
    ...
    with step("Проверка значения response body"):
        assert response_body == register_unsuccess_response_body
    with step("Проверка схемы ответа"):
        validate(response_body, schema=register_user_schemas.post_register_unsuccessful)
    with step("Записываем тело ответа"):
        allure.attach(body=json.dumps(response_body, indent=4, ensure_ascii=True), name="Response",
                      attachment_type=AttachmentType.JSON, extension="json")
